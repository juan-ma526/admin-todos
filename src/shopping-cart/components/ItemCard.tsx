...
export const ItemCard = ({ product, quantity }: Props) => {
  ...
  return (
    <div className="flex items-center shadow rounded-lg w-full bg-gray-800 border-gray-100">
      {/* Product Image */}
      <div className="p-2">
        <Image width={200} height={200} className="rounded" src={product.image} alt={product.name} />
      </div>
      ...
      <div className="px-5 pb-5 w-full flex flex-col mt-2">
        ...
        <div className="flex flex-col items-start justify-between">
          ...
          <span className="font-bold text-white">Total: ${(product.price * quantity).toFixed(2)}</span>
        </div>
      </div>
